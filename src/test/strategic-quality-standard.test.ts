...
describe("strategic quality standard Q0", () => {
  it("keeps both rubrics at 100 points with stable unique IDs", () => {
    expect(rubric.schemaVersion).toBe(1);
    expect(rubric.rubricVersion).toBe("2026-07-16.q0");
    expect(rubric.rubrics).toHaveLength(2);
  ...
  it("maps every critical failure to a deterministic or human check", () => {
    ...
    expect(rubric.criticalFailures).toHaveLength(10);
  ...
  it("keeps committed evaluation material free of real identifiers and credential shapes", () => {
    const committedMaterial = [rubricSource, baselineSource, standardSource, reviewSource].join("\n");
