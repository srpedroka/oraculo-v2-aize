...
const CASE_ID = "Q2B-QUARTERLY-REPEATED-GOAL-004";
...
export async function main() {
  ...
  const summary = await executeCase(item, "Q2B", 1, rubric, {
    runLabel: "q4l",
    reportVersion: "2026-07-17.q4l-smoke",
    ledgerLabel: "Q4L",
  });
  ...
  console.log(`Q4L: ${requiredScores.map((entry) => `${entry.rubricId} ${entry.score}`).join("; ")}; media ${jointAverage.toFixed(2)}.`);
  console.log(`Q4L: custo do smoke US$ ${(summary.generationCostUsd + summary.judgeCostUsd).toFixed(6)}; relatorio privado ${summary.reportPath}.`);
  if (failures.length) throw new Error(`Q4L bloqueada: ${failures.join("; ")}`);
  console.log("Q4L aprovada: a meta recorrente usou a trajetoria, evitou reentrevista e terminou com sintese verificavel em uma confirmacao.");
}
