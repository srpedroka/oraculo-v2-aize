...
export async function main() {
  ...
  const summary = await executeCase(item, "Q2B", 2, rubric, {
    runLabel: "q4q",
    reportVersion: "2026-07-18.q4q-provider-recheck",
    ledgerLabel: "Q4Q",
  });
  ...
  console.log(`Q4Q: ${requiredScores.map((entry) => `${entry.rubricId} ${entry.score}`).join("; ")}; media ${jointAverage.toFixed(2)}.`);
  console.log(`Q4Q: custo do recheck US$ ${(summary.generationCostUsd + summary.judgeCostUsd).toFixed(6)}; relatorio privado ${summary.reportPath}.`);
  if (failures.length) throw new Error(`Q4Q bloqueada: ${failures.join("; ")}`);
  console.log("Q4Q aprovada: o recheck isolado confirmou que o timeout era transitorio e o limite de tres prioridades permaneceu correto.");
}
