...
export async function main() {
  ...
  const summary = await executeCase(item, "Q2B", 1, rubric, {
    runLabel: "q4p",
    reportVersion: "2026-07-18.q4p-smoke",
    ledgerLabel: "Q4P",
  });
  ...
  console.log(`Q4P: ${requiredScores.map((entry) => `${entry.rubricId} ${entry.score}`).join("; ")}; media ${jointAverage.toFixed(2)}.`);
  console.log(`Q4P: custo do smoke US$ ${(summary.generationCostUsd + summary.judgeCostUsd).toFixed(6)}; relatorio privado ${summary.reportPath}.`);
  if (failures.length) throw new Error(`Q4P bloqueada: ${failures.join("; ")}`);
  console.log("Q4P aprovada: a meta de produtividade definiu a medida sem inventar baseline e terminou com uma unica confirmacao.");
}
