...
export async function main() {
  ...
  const summary = await executeCase(item, "Q2B", 1, rubric, {
    runLabel: "q4k",
    reportVersion: "2026-07-17.q4k-smoke",
    ledgerLabel: "Q4K",
  });
  ...
  console.log(`Q4K: ${requiredScores.map((entry) => `${entry.rubricId} ${entry.score}`).join("; ")}; media ${jointAverage.toFixed(2)}.`);
  console.log(`Q4K: custo do smoke US$ ${(summary.generationCostUsd + summary.judgeCostUsd).toFixed(6)}; relatorio privado ${summary.reportPath}.`);
  if (failures.length) throw new Error(`Q4K bloqueada: ${failures.join("; ")}`);
  console.log("Q4K aprovada: CRM foi reenquadrado como meio, o resultado trimestral permaneceu mensuravel e todos os gates passaram.");
}
