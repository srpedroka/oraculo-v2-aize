...
async function resumeQ5AfterCorrection(correctionReference: string) {
  ...
  const normalizedReference = correctionReference.toUpperCase();
  if (!(["Q4N", "Q4O", "Q4P", "Q4Q", "Q4R", "Q4S", "Q4T", "Q4U", "Q4V", "Q4W", "Q4X", "Q4Y", "Q4AG", "Q4AH", "Q4AI", "Q4AJ", "Q4AK", "Q4AL"] as string[]).includes(normalizedReference)) {
    throw new Error("retomada incremental Q5 exige uma correcao ou recheck aprovado (Q4N a Q4Y ou Q4AG a Q4AL)");
  }
  const ledger = await readLedger();
  const progress = await readProgress(ledger.cumulativePlanCostUsd);
  const targetPhase = ["Q4W", "Q4X", "Q4Y"].includes(normalizedReference)
    ? "Q2D"
    : ["Q4U", "Q4V", "Q4AL"].includes(normalizedReference)
      ? "Q2C"
      : "Q2B";
  ...
  progress.baselineVersion = normalizedReference === "Q4AL"
...
async function restartCleanQ5AfterCorrection(correctionReference: string) {
  ...
  if (!nextBaseline) throw new Error("a retomada limpa atual exige a correcao Q4Z, Q4AA, Q4AB, Q4AC, Q4AD, Q4AE ou Q4AF aprovada");
  const ledger = await readLedger();
  const progress = await readProgress(ledger.cumulativePlanCostUsd);
