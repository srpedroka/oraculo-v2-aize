...
Na tela `src/pages/Documents.tsx`, os documentos gravados em `plan_documents` aparecem com filtros por tipo, origem, departamento e periodo. A origem separa documentos de sessao (`session`) de historicos importados (`historical`). O botao **Importar historico** (owner ou coordenador com area gravavel) abre `src/features/history/HistoricalImportDialog.tsx`: upload/drag-and-drop de PDF, PPTX, DOCX, TXT, JPG, PNG e WEBP, texto colado, `suggest-historical-metadata`, previa editavel e `save-historical-document` so apos confirmacao. Nada e gravado ao selecionar o arquivo. Apos salvar, a lista e invalidada e o novo documento e selecionado. A classificacao devolve contrato estruturado (`importSuggestion` com `candidates`, `tables`, `conflicts`, `warnings`) montado em `_shared/historical-import-structure.ts` + `_shared/historical-classifier.ts`. Conflitos obrigatorios exigem escolha humana no dialogo; `save-historical-document` grava `content.import_backup` (schemaVersion 1, sem midia bruta) e permite reabrir a importacao a partir do documento. O componente `src/components/PlanDocument.tsx` renderiza o mesmo conteudo para tela e para a rota limpa `src/pages/DocumentPrint.tsx` (`/documentos/:documentId/imprimir`), que usa CSS de impressao A4 sem sidebar nem painel lateral.
...
- `suggest-historical-metadata`: valida sessao/permissao, restringe areas candidatas ao owner ou ao coordenador da propria area, usa a funcao de IA `background` para sugerir tipo, area, periodo e titulo do historico e cai para heuristica segura se a IA nao estiver disponivel. Nunca inventa periodo quando nao ha data clara. Aceita texto **ou** imagem (JPEG/PNG em base64): com imagem, `callModelWithImage` da funcao `background` transcreve o documento e devolve `extractedText` + sugestao; o binario da imagem nao e persistido. Tabelas com colunas de varios anos sao normalizadas (heuristica + prompt) para linhas mes+ano no `extractedText`/`normalizedText`, com `period` em faixa quando couber.
