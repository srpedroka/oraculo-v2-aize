...
- `_shared/proposals.ts`: aplica propostas confirmadas no banco com validacao server-side de permissao. No trimestral, nao inventa pai anual. No mensal, resolve o pai apenas no trimestre correspondente ao mes e aceita `parent_id` nulo somente com excecao justificada. Ambos persistem baseline, fonte, prazo e acoes estruturadas.
