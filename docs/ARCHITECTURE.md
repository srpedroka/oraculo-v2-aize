...
Na tela `src/pages/Dashboard.tsx`, o bloco "Resultado" combina objetivos ativos com o Dashboard dos 4 KPIs executivos: Faturamento, Margem operacional, Producao e Caixa. As definicoes vivem em `executive_kpis`, os lancamentos mensais em `kpi_monthly_values`, e o frontend renderiza o ano corrente sem seletor de ano na primeira versao. Os minigraficos exibem Meta/Atingido com os meses no eixo horizontal. Owners e admins abrem `src/features/kpi/KpiEditorDialog.tsx` para editar a grade de 12 meses ou importar `.xlsx`, `.xls` e `.csv`: `src/lib/kpiSpreadsheet.ts` le a planilha no navegador, manda apenas a tabela textual para `suggest-kpi-spreadsheet`, recebe uma proposta pela funcao de IA `background` e so grava depois da confirmacao explicita da previa. A escrita vai pelo store direto ao Supabase e depende da RLS `is_admin`.
...
- `suggest-kpi-spreadsheet`: valida sessao e papel `owner`/`admin`, carrega as definicoes dos quatro KPIs e usa a funcao de IA `background` para sugerir Meta/Atingido por indicador e mes. A funcao nao grava valores; a previa precisa ser confirmada no editor e a escrita final continua sob RLS.
