...
- `_shared/proposals.ts`: aplica propostas confirmadas no banco com validacao server-side de permissao. No trimestral, nao inventa pai anual: usa objetivo existente/criado explicitamente ou mantém `parent_id` nulo quando a excecao foi justificada; persiste baseline, fonte, prazo e acoes estruturadas.
- `_shared/plan-documents.ts`: transforma propostas confirmadas em `plan_documents` deterministico, incrementando versao por empresa/area/tipo/periodo. O documento trimestral preserva alinhamento/excecao anual, riscos, trade-offs, cadencia, baseline, fonte e acoes.
