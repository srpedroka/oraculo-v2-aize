...
- `_shared/plan-documents.ts`: transforma propostas confirmadas em `plan_documents` deterministico, incrementando versao por empresa/area/tipo/periodo e preservando rastreabilidade de origem, sessao e tipo de ritual. O trimestral preserva alinhamento/excecao anual; o mensal preserva alinhamento/excecao trimestral, capacidade, backlog, decisoes herdadas, riscos, bloqueios, cadencia e proximo compromisso. A Revisao Semestral usa um pacote canonico unico com duas secoes materialmente distintas, revisao do primeiro semestre e plano do segundo semestre, para impedir divergencia entre tela, PDF e WhatsApp.
...
A Revisao Semestral sob demanda usa o mesmo motor de sessao com `type = strategic_review`, mas tem fronteira mais estreita: apenas owner pode iniciar/gravar a revisao da empresa, sem `area_id`. A IA pode produzir diagnostico do primeiro semestre e prioridades do segundo sem alterar objetivos. A proposta `apply_strategic_review` so atualiza objetivos estrategicos existentes da organizacao quando traz ajustes explicitos nos campos `metric`, `target`, `current`, `deadline` ou `status`, com justificativa obrigatoria. Ao confirmar, o servidor captura snapshot de antes/depois e grava um pacote `plan_documents.type = strategic_review`; sem ajuste explicito, o plano anual permanece inalterado. Retry da confirmacao reutiliza o documento da sessao e nao duplica mutacao.
