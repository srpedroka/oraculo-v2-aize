...
- `_shared/session-engine.ts`: fachada/orquestrador das sessoes de planejamento e confirmação; criação comum fica em `session-runtime.ts`, importações prontas em `session-imports.ts` e normalizadores/prompts puros em `session-ready-plans.ts`. Antes de persistir uma resposta generativa, aplica JSON Schema pelo provedor, deriva `_adaptive` no servidor, canoniza tipo/periodo/escopo e a confirmacao da proposta, permite uma unica autocorrecao contabilizada e usa fallback deterministico quando a segunda resposta ainda viola o contrato. `_shared/session-output-schema.ts`, `_shared/session-canonical-envelope.ts` e `_shared/model-structured-output.ts` materializam essas fronteiras. Depois de uma confirmacao bem-sucedida, app e WhatsApp recebem o documento canonico ligado ao `session_id`; uma repeticao encontra esse mesmo documento e devolve sucesso idempotente sem executar nova mutacao.
...
- `_shared/plan-documents.ts`: transforma propostas confirmadas em `plan_documents` deterministico, incrementando versao por empresa/area/tipo/periodo e preservando rastreabilidade de origem, sessao e tipo de ritual. O trimestral preserva alinhamento/excecao anual; o mensal preserva alinhamento/excecao trimestral, capacidade, backlog, decisoes herdadas, riscos, bloqueios, cadencia e proximo compromisso.
