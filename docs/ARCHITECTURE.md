...
A rota pública `/privacidade` descreve em PT-BR os dados mantidos no Supabase, provedores de IA possíveis, WhatsApp, áudio, arquivos, retenção, backups e controles do titular. A aba `Configurações > Privacidade` mostra os provedores efetivamente selecionados e o estado do WhatsApp. `data_notice_versions` ancora versões publicadas; `organization_data_notice_acknowledgements` registra de forma imutável a ciência da versão pelo owner, uma vez por empresa. Membros leem o status, mas não gravam; o aviso no shell é dispensável por versão e nunca bloqueia login, planejamento ou WhatsApp.
...
Documentos recebidos pelo WhatsApp são baixados e extraídos apenas em memória. A rota da Evolution `/message/downloadmedia` tem precedência sobre URL direta; extensão e MIME são apenas metadados, e o servidor verifica assinatura/UTF-8 dos bytes antes de decidir se a mídia já está aberta ou precisa ser descriptografada com `mediaKey`. TXT e Markdown são decodificados como texto puro; DOCX e PPTX usam seus formatos estruturados; PDF usa `unpdf`/PDF.js no runtime Deno para interpretar a camada de texto real, inclusive streams comprimidos. O classificador de bastidores devolve tanto o destino operacional quanto uma leitura do conteúdo (`documentKind`, resumo, pontos principais e uso sugerido), priorizando o conteúdo sobre o nome. A exceção é uma `strategic_review` ativa: o texto já extraído segue diretamente ao modelo `planning`, em contexto transitório de até 60 mil caracteres, sem depender de `background`; o guard recusa alegação de corrupção depois da extração comprovada. Materiais de apoio como roteiro, ata ou procedimento podem ficar em `unknown` sem serem tratados como "não lidos". Para continuidade da conversa, `chat_messages` recebe somente um resumo automático limitado e marcado como dado não confiável; nome, bytes e texto bruto permanecem fora do histórico.
...
A Fatia 6F transforma essa restauração em prova recorrente. Alterações nas tabelas duráveis exportadas preservam em `organization_backup_requests.requested_at` o instante da primeira mutação ainda não protegida, permitindo medir o RPO de 30 minutos. `Configurações > Backups` mostra RPO/RTO, pendência e último exercício; `Testar recuperação` usa o Storage interno no exercício mensal e obriga download do R2 no exercício trimestral. O clone é validado por checksum, contagens críticas, ausência de chaves e WhatsApp/fila/outbox desligados. `organization_restore_runs` registra origem, duração e resultado estruturado; `Concluir teste` remove somente o clone. `organization_recovery_incidents` guarda ocorrências estruturadas owner-only, sem texto livre ou conteúdo de negócio, e o painel de Saúde permite abrir/resolver a ocorrência sem acrescentar confirmação à rotina normal.
...
- `20260715220000_disaster_recovery.sql`: RPO mensurável, metadados/verificação dos restores, incidentes estruturados owner-only e cobertura de backup para todas as tabelas duráveis exportadas.
...
- `oracle-session`: motor de condução da Fase 2 da V3. Inicia sessoes estruturadas, processa mensagens com a funcao de IA `planning`, importa planos prontos, conduz a Revisao Estrategica `strategic_review`, persiste fase/estado/proposta pendente e grava planos, revisoes ou fechamentos apenas depois de confirmacao. Planos mensais usam o trimestre derivado do mes solicitado, inclusive passado/futuro, e passam por limite server-side de tres resultados e cinco acoes totais.
...
- `operational-health`: monitor protegido por segredo de cron ou autorização de owner. Consolida SLOs de frontend, banco, WhatsApp, backup interno/externo, IA e recuperação em snapshots service-only; também sinaliza retirada incomum em massa, migrations destrutivas auditadas, incidentes estruturados e exercícios mensal/trimestral vencidos, sem conteúdo de negócio. Somente owner abre ou resolve um incidente pelo endpoint; o navegador não escreve diretamente na tabela.
...
- `_shared/plan-context.ts`: monta contexto textual do plano, com empresa, objetivos estrategicos, area em foco, plano anual, trimestre/mes em foco, IDs de objetivos/acoes, acoes-chave, evidencias, pendencias e historicos relevantes. O foco `semester_review` agrega janeiro-junho, T1/T2, todas as areas, KPIs, evidencias, check-ins e documentos pertinentes com limites proprios. Quando existe `plan_documents.type = company_profile` confirmado, injeta o bloco permanente "PERFIL DA EMPRESA" (resumo truncado a ~1200 caracteres) logo após EMPRESA/TEMA, em todos os focos; sem perfil o bloco some sem erro.
- `_shared/session-engine.ts`: fachada/orquestrador das sessoes de planejamento e confirmação; criação comum fica em `session-runtime.ts`, importações prontas em `session-imports.ts` e normalizadores/prompts puros em `session-ready-plans.ts`. A IA interpreta cada turno e possui a resposta visivel; fases e condutores funcionam como referencias internas de qualidade. `_shared/planning-situation.ts` transforma deteccoes deterministas de pendencia, capacidade e fechamento em `kind`, fatos canonicos e decisao para orientar a fala do modelo. Depois da resposta, o servidor reaplica estado, fase, proposta e `done` canonicos; a telemetria guarda somente identificador e contagem da situacao. `_shared/session-conversation.ts` reconhece perguntas sobre o processo e ofertas de arquivo, responde com o mesmo contexto e preserva fase/estado/proposta antes de retomar. `_shared/strategic-review-application.ts` liga uma revisao semestral ja aprovada ao plano anual do mesmo ano, injeta o documento selecionado como contexto nao confiavel e exige cobertura verificavel: cada prioridade precisa de destino em objetivo e cada primeira acao precisa de destino em projeto. Aplicacoes antigas que criaram documento anual sem essa cobertura sao reconhecidas como incompletas e podem ser reparadas sem apagar versoes. Se a fala natural ja apresentou objetivos, projetos, antes/depois e uma confirmacao, a extracao estrutural nao pode aceitar `proposal` nula; uma confirmacao natural recupera a proposta da fala anterior ou da conversa recente, reaplica as mesmas guardas e somente entao a deixa pendente. `_shared/whatsapp-review-document.ts` entrega texto extraído apenas ao turno ativo da revisão e mantém no histórico um recibo sem nome ou bruto; `_shared/whatsapp-media-crypto.ts` separa validação dos bytes e descriptografia da mídia. O servidor continua dono do bloco operacional: aplica JSON Schema, deriva `_adaptive`, canoniza tipo/periodo/escopo, permite uma unica autocorrecao e usa fallback tecnico contextual se o provedor falhar novamente. `_shared/session-output-schema.ts`, `_shared/session-canonical-envelope.ts` e `_shared/model-structured-output.ts` materializam essas fronteiras. Depois de uma confirmacao bem-sucedida, app e WhatsApp recebem o documento canonico ligado ao `session_id`; uma repeticao encontra esse mesmo documento e devolve sucesso idempotente sem executar nova mutacao.
- `_shared/session-adaptive.ts`: contrato e guardas puros da conducao adaptativa. Classifica a resposta como vaga, parcial ou pronta, valida fatos contra chaves concretas do estado da sessao, impede regressao/avanco vazio, repeticao semantica, perguntas multiplas, proposta prematura e exposicao de nomes tecnicos. A camada de naturalidade bloqueia bordao/parafrase, pergunta de campo sem fato e resposta comum longa; no anual, atividade nao pode ser aceita como objetivo, meta autodeclarada pequena precisa ser confrontada, tensao entre receita/margem/capacidade precisa virar escolha com renuncia e um portfolio no limite recebe sintese deterministica a partir da proposta valida, sem regenerar o plano. No trimestral, abertura vaga investiga situacao, causa, impacto e mudanca desejada; uma atividade curta como implantar CRM precisa ser reenquadrada pelo resultado empresarial/adocao antes de campos operacionais; falta de alinhamento anual nao interrompe o diagnostico.
- `_shared/quarterly-guidance.ts`: contrato puro adicional para sessoes trimestrais. Exige de 1 a 3 resultados verificaveis, indicador, baseline, alvo, fonte, prazo, dono, ao menos uma acao completa e alinhamento anual real ou excecao justificada; tambem recusa troca indevida para o ritual anual. Cadencia so e normalizada quando a conversa contem acao explicita de acompanhamento e frequencia explicita, evitando inferencia a partir de mencoes ambiguas.
- `_shared/monthly-guidance.ts`: contrato puro adicional para sessoes mensais. Exige 1 a 3 resultados verificaveis, no maximo 5 acoes completas no plano inteiro, datas dentro do mes, vinculo trimestral real ou excecao justificada e decisao explicita para pendencias herdadas; tambem fornece fallback seguro de capacidade.
...
- `_shared/proposals.ts`: aplica propostas confirmadas no banco com validacao server-side de permissao. No trimestral, nao inventa pai anual; se a proposta vinculada omitir a copia redundante em `annualObjectives`, pode reutilizar apenas o `main_annual_objective_id` canonico da mesma empresa, area e ano, validado por vinculo estrategico ou titulo confirmado. No mensal, resolve o pai apenas no trimestre correspondente ao mes e aceita `parent_id` nulo somente com excecao justificada. Ambos persistem baseline, fonte, prazo e acoes estruturadas. Na aplicacao contextual de uma revisao, repete dentro da transacao a exigencia de `update_current_year`, usa o documento aprovado como fonte imutavel, incorpora deterministicamente contexto/foco/cadencia/riscos/renuncias/aprendizados e aplica as decisoes confirmadas de objetivos e projetos antes de gerar a nova versao canonica.
- `_shared/plan-documents.ts`: transforma propostas confirmadas em `plan_documents` deterministico, incrementando versao por empresa/area/tipo/periodo e preservando rastreabilidade de origem, sessao e tipo de ritual. O trimestral preserva alinhamento/excecao anual; o mensal preserva alinhamento/excecao trimestral, capacidade, backlog, decisoes herdadas, riscos, bloqueios, cadencia e proximo compromisso. A Revisao Estrategica usa um pacote canonico com diagnostico e direcionamento; quando atualiza o plano vigente, a mesma transacao cria uma nova versao completa do documento `strategic`. Tela, PDF e WhatsApp renderizam o mesmo conteudo.
- `_shared/plan-render.ts`: formata o mesmo `content` canonico para WhatsApp, com versao/origem e blocos divididos por `---`; nao consulta IA nem cria uma representacao paralela do plano.
...
A Revisao Estrategica sob demanda usa o mesmo motor de sessao com `type = strategic_review`: apenas owner pode iniciar/gravar a revisao da empresa, sem `area_id`. `review_cycle = midyear` aceita `annual_plan_update.mode = preserve|update_current_year`; a atualizacao pode alterar blocos gerais, atualizar/criar/arquivar objetivos e manter/atualizar/criar/arquivar projetos, sempre com diff e justificativa. Ao aplicar uma revisao aprovada, `sourcePriorityKey` liga cada prioridade a seu objetivo e sua primeira acao ao projeto correspondente; resumo generico isolado nao satisfaz o contrato. Tudo roda no envelope transacional e o arquivamento reutiliza `set_operational_item_archived`, preservando dependencias e reversao. O servidor grava snapshots antes/depois, o documento `strategic_review` e, quando houver mudanca, uma nova versao `strategic`. Em `year_end`, o ano encerrado nao pode ser atualizado: a proposta usa `prepare_next_year` e guarda o briefing para o fluxo anual seguinte. Retry nao duplica documento nem mutacao.
...
Na Fase 6, toda proposta confirmada (`save_strategic_plan`, `save_quarterly_plan`, `save_monthly_plan`, `month_close`, `quarter_close`) tambem gera um documento canonico em `plan_documents`. O documento nao depende de nova chamada de IA: e montado de forma deterministica por `_shared/plan-documents.ts` a partir da proposta aprovada e de dados basicos de empresa/area. A tela `/documentos`, o PDF A4 e o resumo nativo do WhatsApp leem o mesmo `content`; a Q4E adicionou um teste de igualdade semantica que compara proposta, banco, documento e 18 fatos materiais nas tres saidas, inclusive hierarquia, prazo, baseline, meta, fonte, dono e acoes. Renderizar nao grava, nao incrementa versao e nao chama provedor.
...
Na síntese de uma sessão, o condutor apresenta o resumo e a proposta na mesma resposta e pede uma única confirmação server-side. Frases intermediárias como "pode gerar" ou "está bom" não criam uma segunda revisão; depois que `pending_proposal` existe, a próxima confirmação válida grava de forma idempotente. No app, `OraclePanel` mantém uma sessão selecionada explicitamente, mostra ritual/empresa/area/periodo/fase, filtra as mensagens pela conversa vinculada e renderiza a proposta dos seis rituais no mesmo shell. Ajustar e descartar preservam a proposta ate o aceite do servidor; o sucesso oferece o documento gravado. No WhatsApp, a apresentacao continua textual, mas a regra de confirmacao e o documento devolvido sao os mesmos.
