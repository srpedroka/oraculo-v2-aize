...
Na Fatia 3A, existe uma entrada durável opcional antes do passo 4. A flag `whatsapp_settings.inbound_queue_enabled` nasce `false`, só pode ser alterada pelo `service_role` e não entra ativa em restaurações. A Fatia 3B adiciona o worker: claim usa `FOR UPDATE SKIP LOCKED`, uma conversa não processa dois jobs ao mesmo tempo, locks abandonados voltam para retry e a quinta falha vira `dead`. No piloto, somente texto entra nessa fila. Mídia usa o handler síncrono porque o download da Evo Go exige a mensagem original e a política de segurança proíbe persistir `mediaKey`, URL temporária ou arquivo bruto.
...
Pedidos explícitos de novo planejamento são resolvidos antes da conversa diária. A sessão é identificada por empresa, pessoa, tipo, período e área; mensal/trimestral/fechamento nunca iniciam sem área. Ao trocar de plano, uma sessão sem proposta pendente é marcada como abandonada e a nova sessão é criada no mesmo fluxo. Uma proposta pronta precisa ser confirmada ou descartada antes da troca.

Documentos canônicos são recuperados somente por correspondência exata de tipo, área e período, usando primeiro a sessão mais recente da conversa. Não existe fallback para qualquer área ou período. Quando a pessoa pede o arquivo ou confirma um plano pelo WhatsApp, `_shared/plan-pdf.ts` gera um PDF em memória e `_shared/whatsapp.ts` envia pela rota de mídia da Evolution; bytes/base64 não são gravados no banco.
