...
- `_shared/session-engine.ts`: fachada/orquestrador das sessoes de planejamento e confirmação; criação comum fica em `session-runtime.ts`, importações prontas em `session-imports.ts` e normalizadores/prompts puros em `session-ready-plans.ts`. Antes de persistir uma resposta generativa, aplica o contrato adaptativo, permite uma unica autocorrecao contabilizada e usa fallback deterministico quando a segunda resposta ainda viola o contrato.
