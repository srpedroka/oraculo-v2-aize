...
- `_shared/session-adaptive.ts`: contrato e guardas puros da conducao adaptativa. Classifica a resposta como vaga, parcial ou pronta, valida fatos contra chaves concretas do estado da sessao, impede regressao/avanco vazio, repeticao semantica, perguntas multiplas, proposta prematura e exposicao de nomes tecnicos. A camada de naturalidade bloqueia bordao/parafrase, pergunta de campo sem fato e resposta comum longa; no anual, atividade nao pode ser aceita como objetivo, meta autodeclarada pequena precisa ser confrontada, tensao entre receita/margem/capacidade precisa virar escolha com renuncia e um portfolio no limite recebe sintese deterministica a partir da proposta valida, sem regenerar o plano.
- `_shared/quarterly-guidance.ts`: contrato puro adicional para sessoes trimestrais. Exige de 1 a 3 resultados verificaveis, indicador, baseline, alvo, fonte, prazo, dono, ao menos uma acao completa e alinhamento anual real ou excecao justificada; tambem recusa troca indevida para o ritual anual.
