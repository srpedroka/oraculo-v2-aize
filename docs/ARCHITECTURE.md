...
A Revisao Estrategica sob demanda usa o mesmo motor de sessao com `type = strategic_review`: apenas owner pode iniciar/gravar a revisao da empresa, sem `area_id`. `review_cycle = midyear` aceita `annual_plan_update.mode = preserve|update_current_year`; a atualizacao pode alterar blocos gerais e atualizar, criar ou arquivar objetivos, sempre com diff e justificativa. Tudo roda no envelope transacional e o arquivamento reutiliza `set_operational_item_archived`, preservando dependencias e reversao. O servidor grava snapshots antes/depois, o documento `strategic_review` e, quando houver mudanca, uma nova versao `strategic`. Em `year_end`, o ano encerrado nao pode ser atualizado: a proposta usa `prepare_next_year` e guarda o briefing para o fluxo anual seguinte. Retry nao duplica documento nem mutacao.
