...
A Fatia 6A documenta o comportamento atual sem escolher base legal nem papel contratual em nome do responsável. A Fatia 6B publica um aviso operacional neutro e registra ciência versionada pelo owner; esse registro não é consentimento genérico nem substitui a validação formal de controlador/operador/suboperadores, contato institucional, finalidade/base legal e contratos com Supabase, Netlify, Evolution/VPS, provedores de IA e R2. Arquivos brutos processados em memória também entram no inventário; ausência de persistência não significa ausência de tratamento ou transferência.
...
A fila de entrada é service-only: `anon` e `authenticated` não leem a tabela nem executam RPCs, e o navegador não pode alterar `inbound_queue_enabled`. Eventos sem autenticação e mensagens `from_me` são descartados antes do enfileiramento. O payload usa allowlist escalar e nunca aceita mídia/base64, URL temporária, `mediaKey`, `directPath`, header ou segredo. Texto fica limitado a 12 mil caracteres. Desde a Fatia 3E, texto não possui fallback síncrono: fila ou outbox indisponível produz `503`, sem gravar mensagem ou chamar IA. Áudio e documento não entram na fila: são processados sincronamente com o payload autenticado original, porque a Evo Go precisa do descritor criptográfico para baixar a mídia e ele não pode ser persistido. Jobs antigos de mídia continuam suportados apenas para drenagem/compatibilidade e nunca recebem conteúdo bruto.

O `whatsapp-worker` não confia em sessão de usuário: é público no gateway para permitir `pg_net`, mas exige `x-oraculo-worker-secret` comparado em tempo constante com `whatsapp_worker_secrets`, tabela inacessível a membros e owners. Endpoint inválido não vira processamento global. Claim, heartbeat, conclusão, falha e limpeza são RPCs revogadas de `anon`/`authenticated`. Erros persistidos removem URL, bearer, tokens e caracteres de controle. Jobs concluídos ficam 24 horas; dead-letter, 7 dias. O endpoint automático nasce nulo, e produção permanece sem ativação até o teste controlado da 3B.

O monitor `operational-health` também é público apenas no gateway para receber `pg_net`: cron exige `x-oraculo-monitor-secret` em comparação constante e consultas interativas exigem JWT + owner. `operational_health_snapshots`, `operational_alerts`, `operational_monitor_secrets`, `operational_safety_events` e `ai_function_errors` são service-only. Métricas não incluem prompt, resposta, conversa, documento, telefone, email, URL temporária ou mensagem de erro do provedor. A S4 acrescenta alertas para réplica externa ausente/atrasada/falha, 20 ou mais arquivamentos em 15 minutos, migration destrutiva auditada, restauração mensal vencida e exercício trimestral vencido. Eles informam e não bloqueiam a operação.

Relatos do Error Boundary aceitam qualquer membro autenticado somente para a própria empresa. `frontend_error_events` é service-only e guarda apenas código de ocorrência, tipo técnico do erro e caminho sem query string; mensagem, stack, estado React e conteúdo do usuário são descartados no navegador. O código exibido ao usuário permite correlação sem expor detalhes internos.

A outbox de saída segue a mesma fronteira: `whatsapp_outbox`, `whatsapp_sender_secrets` e todas as RPCs são service-only; o navegador não pode ler mensagens pendentes, obter o segredo ou alterar `outbound_outbox_enabled`. Respostas de membros conhecidos são divididas em até três blocos e gravadas com `chat_messages` na mesma transação. O sender exige `x-oraculo-sender-secret`, confirma apenas HTTP 2xx e persiste somente status, ID e erro sanitizados. Itens enviados ficam 24 horas; dead-letter, 7 dias. A Fatia 3E torna a outbox obrigatória para respostas textuais normais e o worker recusa processar um job antes de qualquer mutação se ela estiver indisponível. A Evolution não aceita chave de idempotência no envio de texto: se o processo cair depois de o provedor aceitar e antes de gravar o 2xx, o estado é ambíguo e um retry pode duplicar essa única mensagem. Não prometer exatamente-uma-vez sem suporte do provedor.
...
Quando a transcrição ou a leitura de documento falhar, a resposta visível deve ser natural e não expor código técnico. Etapa, status HTTP, tipo, tamanho e assinatura curta podem aparecer somente em logs sanitizados para diagnóstico. Esses logs nunca incluem conteúdo da fala, chave da Evolution, chave OpenAI, segredo de webhook, URL temporária completa ou payload completo.

Arquivos enviados pelo WhatsApp sao processados em memoria para extração de texto e classificação do plano correto. Extensão e MIME não autenticam o conteúdo: antes de pular a descriptografia, o servidor exige assinatura PDF/ZIP real ou UTF-8 plausível; mídia opaca com `mediaKey` é descriptografada em memória. O arquivo bruto nao deve ser salvo no banco, versionado no repositorio nem enviado ao frontend. Para permitir perguntas seguintes sem reter o documento, o historico guarda somente natureza, resumo e ate cinco pontos gerados pela IA, com limites server-side e marcação explícita de dado não confiável; nome e texto bruto ficam omitidos. Durante `strategic_review`, o texto extraído pode entrar somente no contexto transitório do turno, limitado a 60 mil caracteres, e fundamentar o estado resumido produzido pelo condutor; a classificação auxiliar não é autoridade nem pré-requisito. O prompt é instruído a tratar esse bloco apenas como conteúdo e nunca executar comandos vindos dele. Documentos sensiveis continuam sendo dados privados da empresa. Quando o documento for classificado como Plano Estrategico, Trimestral ou Mensal, o texto extraido gera uma proposta pendente em `planning_sessions`; a escrita real no banco continua exigindo confirmação explicita do usuario e validação server-side.

O diagnóstico de saúde usa `whatsapp_health_events`, tabela com RLS e grants exclusivos de `service_role`. Ela guarda somente tipo/origem do evento, IDs técnicos, código de erro sanitizado, status HTTP e horário; não guarda telefone, conteúdo, URL, header, chave, segredo ou corpo da Evolution. A Edge Function `whatsapp-health` exige JWT e owner. Teste e reprocessamento também exigem `aal2` quando a política opcional da empresa estiver ligada. A URL consultada é validada contra protocolos HTTP(S) e destinos locais/IPs privados literais antes do fetch; respostas remotas são reduzidas a estados e códigos seguros.

Reabrir um dead-letter é uma RPC service-only e não liga o mecanismo durável. A função só permite o retry quando a flag e o endpoint da fila correspondente já estão ativos; isso evita deixar item reaberto sem processador. O painel publicado na Fatia 3D mantém `inbound_queue_enabled`, `outbound_outbox_enabled` e endpoints exatamente como estavam.
...
Documentos padronizados da Fase 6 e historicos importados ficam em `public.plan_documents` como JSON (`content`). Eles podem conter objetivos, metas, donos, prazos, aprendizados, decisoes, texto historico da empresa e a classificacao revisada pelo usuario em `content.classification`, portanto sao dados privados protegidos por RLS. Documentos gerados por sessao preservam em `content.rastreabilidade` apenas origem, ID da sessao e tipo do ritual; esse dado segue o mesmo RLS do documento, nao e segredo e nao vira instrucao para a IA. Eles nunca devem conter arquivo bruto, audio bruto, URL temporaria de midia, `mediaKey`, chave de IA, chave da Evolution, senha ou segredo de webhook. PDF, tela e WhatsApp sao renderizacoes somente leitura do mesmo conteudo canonico; nenhum PDF gerado pelo usuario e salvo automaticamente no banco e nenhuma renderizacao pode mutar plano ou chamar provedor.
...
O ciclo de vida da própria empresa segue a mesma trava. `authenticated` não pode mais dar `DELETE` em `public.organizations` (a policy `organizations_delete_owner` foi removida e o privilégio revogado); sair, arquivar, restaurar e excluir passam pela Edge Function `organization-lifecycle`. Sair usa `remove_organization_member` para a própria membership (o único owner é bloqueado). Arquivar/restaurar usa `set_organization_archived` (reversível; arquivar pausa o WhatsApp sem apagar segredo). A exclusão definitiva (`delete_organization_permanently`, exclusiva de `service_role`) só roda com a empresa arquivada, confirmação final booleana, um backup `completed` recente e o nome digitado conferindo; MFA/AAL2 também é exigido quando a política opcional da empresa estiver ligada. Ela revoga chaves de IA, apaga `whatsapp_instance_keys`, limpa apenas os objetos internos de backup e então remove a empresa em cascata; a réplica externa imutável permanece. `organization_lifecycle_audit` registra quem/quando/por quê e é intencionalmente sem FK para `organizations`, então a linha de `permanent_delete` sobrevive à exclusão; a leitura é restrita ao owner enquanto a empresa existir.
...
Respostas das sessoes tambem passam pelo contrato server-side de `_shared/session-adaptive.ts` antes de atualizar `planning_sessions`. Quando o provedor suporta, `_shared/model-structured-output.ts` solicita o envelope por JSON Schema; isso reduz JSON livre, mas nunca substitui a validacao local. O servidor remove campos de escopo sugeridos pelo modelo, reaplica periodo/ano da sessao, deriva `_adaptive` apenas de estado canonico e controla a confirmacao final. Fatos declarados pela IA precisam apontar para chaves concretas do estado; avancos sem evidencia, regressao, repeticao de pergunta, estado tecnico visivel, bordao mecanico e proposta antes de `ready` sao recusados. No anual, atividade aceita como objetivo, meta pequena sem desafio, salto de causa concreta para direcionador generico e tensao de crescimento reduzida a outro menu tambem exigem reparo. Quando uma proposta completa ja e valida, o servidor pode substituir apenas a sintese/confirmacao por uma projecao deterministica dos mesmos dados, evitando nova chamada e sem alterar a proposta. O servidor permite no maximo uma nova chamada de reparo, registrada separadamente em `ai_usage_logs`; se ela continuar invalida, remove proposta prematura e usa resposta deterministica. Texto visivel seguro pode ser preservado mesmo quando apenas o estado interno e corrigido. Envelope ainda malformado falha fechado sem atualizar `state`, `phase`, `pending_proposal` ou dados de plano; a mensagem recebida permanece no historico para permitir nova tentativa. Erros de `oracle-session` expõem somente um `errorCode` tecnico sanitizado, nunca mensagem de banco, conteúdo ou segredo.

Sessoes trimestrais somam o contrato `_shared/quarterly-guidance.ts`. Uma proposta so fica pendente quando contem no maximo tres objetivos verificaveis, acoes completas e alinhamento anual real ou excecao justificada. A confirmacao consulta referencias por empresa e a gravacao recusa vinculo ausente sem excecao; portanto o modelo nao pode fabricar um pai anual para satisfazer o schema. Acoes, baseline, fonte e prazo entram na mesma transacao idempotente do plano e do documento canonico.

Sessoes mensais somam `_shared/monthly-guidance.ts`. O servidor deriva o trimestre do mes da sessao, exige de um a tres resultados, limita cinco acoes no plano inteiro e recusa datas fora do mes. A confirmacao resolve o objetivo trimestral por `org_id`, area, nivel e periodo; nao cria objetivo generico. Sem trimestre, somente uma excecao justificada permite `parent_id` nulo. Pendencias herdadas exigem origem, motivo e decisao entre rolar, renegociar, cortar ou backlog. Se o modelo insistir em resposta generica, o fallback deterministico pergunta esses destinos sem mutar plano.

No fluxo de plano pronto, seja estrategico, trimestral ou mensal, texto de arquivo e nome do arquivo são conteúdo não confiável. O bloco pode orientar a proposta, mas não pode substituir regras, exigir exposição de segredo, abrir URLs, decodificar payloads ou pular confirmação. A saída do modelo passa por limites de profundidade, nós, listas e texto, rejeita chaves perigosas e exige o tipo de proposta correto. Vínculos de objetivo estratégico são consultados por `org_id`, nível e estado ativo antes de aceitar a proposta e novamente dentro da confirmação transacional. O histórico registra apenas um recibo do arquivo, não o texto bruto. Quando um documento chega durante uma `strategic_review` ativa, somente o resumo seguro entra no histórico; o texto extraido completo existe em memoria apenas durante a chamada que continua a revisão e não é persistido em fila, sessão, documento ou log.
...
Revisao Semestral (`strategic_review`) e um ritual owner-only no escopo da empresa. O backend bloqueia inicio com `area_id` ou por usuario que nao seja `owner`, e a confirmacao passa novamente por `assertProposalPermission`. A IA pode propor diagnostico e prioridades sem mutar o plano anual. A proposta `apply_strategic_review` nao pode criar, excluir, renomear ou trocar objetivos em massa: ela so atualiza objetivos estrategicos existentes da mesma organizacao e sem area, nos campos `metric`, `target`, `current`, `deadline` e `status`. Cada ajuste precisa de justificativa; sem ajuste explicito o objetivo permanece inalterado. Ao confirmar, o servidor grava snapshot antes/depois em `plan_documents.type = strategic_review`, e o retry encontra o documento da mesma sessao em vez de duplicar a aplicacao.
...
Integração, segurança e E2E autenticado possuem trava explícita contra a referência de produção. `.agents-private/agent-env` contém somente credenciais de staging e integrações operacionais. O caminho rotineiro de publicação usa o GitHub Environment `production`: o owner autoriza explicitamente o disparo na conversa, e o agente só então inicia manualmente o workflow; o preflight valida SHA, `CI required`, Functions e migrations sem receber segredo. O Environment não exige uma segunda aprovação por reviewer, mas mantém os segredos isolados e aceita somente a branch `main`. Migrations destrutivas são recusadas por padrão e migrations pendentes fora do pacote aprovado nunca entram no `db push`. O token no Chaves do macOS entra apenas no subprocesso local allowlisted de emergência. A fábrica falha visivelmente se não remover organização ou usuário descartável. Consulte `docs/TESTING.md`, `docs/CI.md` e `docs/RUNBOOK.md`.
...
## Conta pessoal e desligamento

A Function `personal-account` é a única fronteira para exportação pessoal e exclusão de Auth. Ela sempre deriva a pessoa do JWT, nunca aceita `user_id` do navegador e registra a solicitação em `personal_data_requests`, tabela sem acesso para `anon` ou `authenticated`.
