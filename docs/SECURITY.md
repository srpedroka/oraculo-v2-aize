...
O monitor `operational-health` também é público apenas no gateway para receber `pg_net`: cron exige `x-oraculo-monitor-secret` em comparação constante e consultas interativas exigem JWT + owner. `operational_health_snapshots`, `operational_alerts`, `operational_monitor_secrets` e `ai_function_errors` são service-only. Métricas não incluem prompt, resposta, conversa, documento, telefone, email, URL temporária ou mensagem de erro do provedor.
