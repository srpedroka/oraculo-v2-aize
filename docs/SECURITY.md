...
A fila de entrada da Fatia 3A é service-only: `anon` e `authenticated` não leem a tabela nem executam a RPC, e o navegador não pode ligar `inbound_queue_enabled`. Eventos sem autenticação e mensagens `from_me` são descartados antes do enfileiramento. O payload usa allowlist escalar por tipo e nunca aceita mídia/base64, URL temporária, `mediaKey`, `directPath`, header ou segredo. Texto fica limitado a 12 mil caracteres e expira em 7 dias; jobs de mídia guardam apenas metadados/localizador mínimo e expiram em 24 horas. A limpeza efetiva e o processamento serão implementados no worker; até lá a flag permanece desligada em produção.
