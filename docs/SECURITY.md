...
A fila de entrada é service-only: `anon` e `authenticated` não leem a tabela nem executam RPCs, e o navegador não pode ligar `inbound_queue_enabled`. Eventos sem autenticação e mensagens `from_me` são descartados antes do enfileiramento. O payload usa allowlist escalar por tipo e nunca aceita mídia/base64, URL temporária, `mediaKey`, `directPath`, header ou segredo. Texto fica limitado a 12 mil caracteres; jobs de mídia guardam apenas metadados/localizador mínimo.
