...
A fila de entrada é service-only: `anon` e `authenticated` não leem a tabela nem executam RPCs, e o navegador não pode ligar `inbound_queue_enabled`. Eventos sem autenticação e mensagens `from_me` são descartados antes do enfileiramento. O payload usa allowlist escalar e nunca aceita mídia/base64, URL temporária, `mediaKey`, `directPath`, header ou segredo. Texto fica limitado a 12 mil caracteres. Áudio e documento não entram na fila: são processados sincronamente com o payload autenticado original, porque a Evo Go precisa do descritor criptográfico para baixar a mídia e ele não pode ser persistido. Jobs antigos de mídia continuam suportados apenas para drenagem/compatibilidade e nunca recebem conteúdo bruto.
...
A outbox de saída segue a mesma fronteira: `whatsapp_outbox`, `whatsapp_sender_secrets` e todas as RPCs são service-only; o navegador não pode ler mensagens pendentes, obter o segredo ou ligar `outbound_outbox_enabled`. Respostas de membros conhecidos são divididas em até três blocos e gravadas com `chat_messages` na mesma transação. O sender exige `x-oraculo-sender-secret`, confirma apenas HTTP 2xx e persiste somente status, ID e erro sanitizados. Itens enviados ficam 24 horas; dead-letter, 7 dias. A estrutura está publicada em produção, porém endpoint e flags permanecem desligados até teste real controlado e autorização. A Evolution não aceita chave de idempotência no envio de texto: se o processo cair depois de o provedor aceitar e antes de gravar o 2xx, o estado é ambíguo e um retry pode duplicar essa única mensagem. Não prometer exatamente-uma-vez sem suporte do provedor.
