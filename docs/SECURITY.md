...
Respostas das sessoes tambem passam pelo contrato server-side de `_shared/session-adaptive.ts` antes de atualizar `planning_sessions`. Fatos declarados pela IA precisam apontar para chaves concretas do estado; avancos sem evidencia, regressao, repeticao de pergunta, estado tecnico visivel e proposta antes de `ready` sao recusados. O servidor permite no maximo uma nova chamada de reparo, registrada separadamente em `ai_usage_logs`; se ela continuar invalida, remove proposta prematura e usa resposta deterministica. Envelope ainda malformado falha fechado sem atualizar `state`, `phase`, `pending_proposal` ou dados de plano; a mensagem recebida permanece no historico para permitir nova tentativa.
