...
- Etapa 7 / preparação 7A, cenário funcional 7B e falhas 7C concluídos no staging em 2026-07-15. O baseline persistente usa duas empresas e cinco usuários descartáveis, MFA somente no owner de teste, IA em monitor e WhatsApp sintético. Históricos, planos, confirmação idempotente, revisão, KPIs, arquivo, auditoria e clone de backup passaram. A 7C comprovou deduplicação, retry/dead-letter, rollback, concorrência, isolamento, papéis, limites, recuperação de tela, backup externo indisponível e JWT; o baseline posterior passou 12/12 com WhatsApp real inerte. A geração pelo modelo ficou explicitamente não exercitada porque o staging não tem chave própria de IA; nenhuma chave de produção foi copiada. Comandos e regra de limpeza estão em `docs/MASTER_TEST.md`; `.agents-private/master-test-7a.json`, `master-test-7b.json` e `master-test-7c.json` devem ser preservados até o aceite final 7D.
...
- Executar a 7D do Teste Mestre: E2E desktop/mobile, revisão visual, smoke somente leitura em produção, relatório consolidado e limpeza do baseline somente após aceite explícito do owner.
