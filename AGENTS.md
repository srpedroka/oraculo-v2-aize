...
- Etapa 7 / preparação 7A e cenário funcional 7B concluídos no staging em 2026-07-15. O baseline persistente usa duas empresas e cinco usuários descartáveis, MFA somente no owner de teste, IA em monitor e WhatsApp sintético. Históricos, planos, confirmação idempotente, revisão, KPIs, arquivo, auditoria e clone de backup passaram. A geração pelo modelo ficou explicitamente não exercitada porque o staging não tem chave própria de IA; nenhuma chave de produção foi copiada. Comandos e regra de limpeza estão em `docs/MASTER_TEST.md`; `.agents-private/master-test-7a.json` e `.agents-private/master-test-7b.json` devem ser preservados até o aceite final.
